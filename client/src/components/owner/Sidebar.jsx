...
const Sidebar = () => {
    ...
    const updateImage=async ()=>{
        try {
            ...
            const {data}=await axios.post('/api/owner/update_image',formData)
            if(data.success){
                fetchUser()
  ...
  return (
    <div className='relative min-h-screen md:flex flex-col items-center pt-8 max-w-13 md:max-w-60 w-full border-r border-borderColor text-sm'>
        ...
        <div className='group relative'>
            <label htmlFor="image">
                <img className='h-9 md:h-14 w-9 md:w-14 rounded-full mx-auto' src={image? URL.createObjectURL(image):user?.image || "https://image.unsplash.com/photo-1633332755192-727a05c4013d?q=80&w=300"} alt=""/>
        ...
        <div className='w-full'>
            {ownerMenuLinks.map((link,index)=>(
                <NavLink key={index} to={link.path} className={`relative flex items-center gap-2 w-full py-3 pl-4 first:mt-6 ${link.path===location.pathname? 'bg-primary/10 text-primary':'text-gray-600'}`}>
                    ...
                    <span className='max-md-hiddden'>{link.name}</span>
